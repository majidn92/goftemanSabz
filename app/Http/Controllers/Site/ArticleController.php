...
class ArticleController extends Controller
{
	public function show($id)
	{
		...
		$side_ads = SideAd::orderBy('rank', 'asc')->get();

		return view('site.pages.article_detail', compact('post', 'widgets', 'socialLinks', 'relatedPost', 'tracks', 'post_contents', 'schema', 'side_ads'));
	}
	...
	{
		try {
			$posts = Post::where(DB::raw('LOWER(title)'), 'like', '%' . $request->get('search') . '%')
				->when(Sentinel::check() == false, function ($query) {
					$query->where('auth_required', 0);
				})
				->orderBy('created_at', 'desc')->limit(6)->get();

			$totalPostCount = Post::where(DB::raw('LOWER(title)'), 'like', '%' . $request->get('search') . '%')
				->when(Sentinel::check() == false, function ($query) {
					$query->where('auth_required', 0);
				})->count();
			// dd($totalPostCount);
	...
	public function postByCategory($slug)
	{
		try {
			...
			if (Sentinel::check() == false) {
				$posts = $posts->where('auth_required', 0);
			}
			...
			return view('site.pages.category_posts', compact('posts', 'widgets', 'totalPostCount', 'id', 'category'));
		} catch (\Exception $e) {
			return view('site.pages.404');
		}
	}
	...
	{
		$skip = ($request->last_id * 6) + 6;

		$sub_category = SubCategory::find($request->sub_category_id);
		$posts = $sub_category->post;
		$posts = $posts->where('visibility', 1)
			->where('status', 1)
			->where('language', LaravelLocalization::setLocale() ?? settingHelper('default_language'))
			->sortByDesc('id');
		...
		foreach ($posts as $post) {
			...
			//            $appendRow .= "<div class='sub_category'>";
			//            $appendRow .= "<ul class='global-list'>";
			//            if ($post->sub_category != "") :
			//                $appendRow .= "<li><a href='" . url('sub_category', $post->sub_category->slug) . "'> " . $post->sub_category->sub_category_name . "</a></li>";
			//            endif;
			//            $appendRow .= "</ul>";
			//            $appendRow .= "</div>";
			$appendRow .= "</div>";
			...
			$appendRow .= "<a href=' " . route('article.detail', ['id' => $post->slug]) . " '> " . \Illuminate\Support\Str::limit($post->title, 85) . " ";
			$appendRow .= "</a>";
			$appendRow .= "</h3>";

			// حذف نمایش نویسنده و تاریخ خبر
			// $appendRow .= "<div class='entry-meta mb-2'>";
			// $appendRow .= "<ul class='global-list'>";
			// $appendRow .= "<li> " . " <a href='" . route('site.author', ['id' => $post->user->id]) . "'> " . data_get($post, 'user.first_name') . data_get($post, 'user.last_name') . " </a></li>";
			// $appendRow .= "<li><a href='" . route('article.date', date('Y-m-d', strtotime($post->updated_at))) . "'> " . miladi_to_jalali($post->updated_at) . "</a></li>";
			// $appendRow .= "</ul>";
			// $appendRow .= "</div> ";
			$appendRow .= "<p>" . (\Illuminate\Support\Str::limit($post->sub_title, 160)) . "</p>";
			$appendRow .= "</div>";
			$appendRow .= "</div>";
	...
	public function postBySubCategory($slug)
	{
		try {
			...
			return view('site.pages.sub_category_posts', compact('posts', 'widgets', 'totalPostCount', 'id', 'sub_category', 'category'));
		} catch (\Exception $e) {
			return view('site.pages.404');
		}
	}

	public function getReadMorePostSubcategory(Request $request)
	{
		$skip = ($request->last_id * 6) + 6;
	...
	public function postByTags($slug)
	{
		...
		try {
			...
			return view('site.pages.tags_posts', compact('posts', 'widgets', 'totalPostCount', 'slug', 'result'));
		} catch (\Exception $e) {
			return view('site.pages.404');
		}
	}
	...
	public function postByAuthor($id)
	{
		try {
			...
			return view('site.pages.category_posts', compact('posts', 'widgets', 'id'));
		} catch (\Exception $e) {
			return view('site.pages.404');
		}
	}
