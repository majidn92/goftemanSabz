...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $messages = [
            '*.required'            => 'پر کردن این ورودی الزامی است',
        ];

        Validator::make($request->all(), $rules, $messages)->validate();

        $request['is_password_set'] = 1;
        $request['dob'] = jalali_to_miladi($request->dob);
        ...
        try {
            ...
            return \redirect()->back()->with('success', 'با موفقیت ثبت نام شد');
        } catch (\Exception $e) {
    ...
    public function userList()
    {
        ...
        $roles      = Role::allRole();
        $users      = User::with(['withRoles', 'withActivation', 'image'])->orderBy('id','desc')->paginate(10);
    ...
    {
        $messages = [
            'profile_image.mimes' => 'عکس انتخابی باید از نوع jpg یا png باشد'
        ];
        ...
        $user->is_featured = 0;
        $user->featured_order = 1;
        $user->first_name           = $request->first_name;
        $user->last_name            = $request->last_name;
        $user->phone                = $request->phone;
        // $user->dob                  = jalali_to_miladi($request->dob);
        if(isset($request->is_featured)){
            $user->is_featured = 1;
            $user->featured_order = $request->featured_order;
        }
        $user->save();
