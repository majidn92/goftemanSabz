...
class MenuItemController extends Controller
{
    public function menuItem()
    {
        ...
        $posts = Post::select('id', 'title')->orderBy('id', 'desc')->where('language', $selectedLanguage)->get();
    ...
    // متدهای مربوط به منو فوتر 
    public function footer_menu_item()
    {
        $items = \DB::table('footer_menu_items')->orderBy('rank')->paginate(10);
        return view('appearance::footer_menu', compact('items'));
    }

    public function footer_menu_store(Request $request)
    {
        $data = $request->except('_token');
        \DB::table('footer_menu_items')->insert($data);
        return redirect('footer-menu')->with('success', __('successfully_added'));
    }

    public function footer_menu_update(Request $request)
    {
        $id = $request->id;
        $data = $request->except('id', '_token');
        \DB::table('footer_menu_items')->where('id', $id)->update($data);
        return redirect('footer-menu')->with('success', __('successfully_updated'));
    }

    public function footer_menu_delete(Request $request)
    {
        $id = $request->id;
        \DB::table('footer_menu_items')->where('id', $id)->delete();
        $response = [
            'status' => 'success',
