<div id="audio-gallery" class="modal fade audio-modal-lg" tabindex="-1" role="dialog" aria-labelledby="audioGallery" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content">
                ...
                <div class="row">
                    <div class="col-md-4 border-right">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                <label for="audio" class="upload-file-btn btn btn-primary  btn-block"><i class="fa fa-folder input-file" aria-hidden="true"></i> {{ __('add_audio') }}
                                </label>
                                <input id="audio" name="audio" type="file" class="form-control d-none" required onChange="audioUploadBtn()" data-index="0">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group" id="divAudioUploadBtn">
                                <div class="form-group text-center">
                                    <button type="submit" name="btn_audio_upload" id="audio-upload-btn" class="btn btn-primary btn-block"><i class="fas fa-cloud-upload-alt"></i> {{ __('upload') }}</button>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="ajax-loading" id="ajax-audio-loading"><img src="{{ static_asset('site/images/preloader-2.gif') }}" />
                        </div>
                        <div class="load-more" id="load-more-audio"><a href="javascript:void(0)" class="">{{ __('load_more') }}</a></div>
            ...
            <div class="modal-footer">
                <button type="button" id="selectAudio" class="btn btn-primary" data-dismiss="modal">{{ __('select_audio') }}</button>
...
<script>
    $(document).ready(function(e) {
        ...
        $('#audioUploadForm').on('submit', (function(e) {
                ...
                $("#audio-upload-btn").html('<i class="fa fa-spinner fa-pulse fa-fw"></i><span class="sr-only"></span> در حال بارگذاری ...');
                $.ajaxSetup({
                    headers: {
                ...
                $.ajax({
                        ...
                        @endif
                        $.notify('صوت با موفقیت بارگذاری شد', "success");
                        $("#audio").val('');
...
<script>
    ...
    $("#selectAudio").on('click', function() {
        ...
        var url = `{{ url('fetch-audio-selected/${selected_audio_id}') }}`;
        $.ajax({
            type: "get",
            url: url,
            success: function(response) {
                $("#audio_preview").attr('src', response);
            }
        });
