<div id="media-gallery" class="modal fade image-modal-lg" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
                ...
                <div class="row">
                    <div class="col-md-4 border-right">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                <label for="image" class="upload-file-btn btn btn-primary  btn-block">
                            ...
                            <div class="form-group">
                                <div class="form-group text-center">
                                    <img src="{{ static_asset('default-image/default-100x100.png') }} " id="perview_current_image" data-index="0" width="200" height="200" alt="image" class="img-responsive img-thumbnail">
                                </div>
                            </div>
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="load-more" id="load-more-image"><a href="javascript:void(0)" class="">{{ __('load_more') }}</a></div>

                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" id="selectImage" class="btn btn-primary selectImage ml-4" data-dismiss="modal">{{ __('select_image') }}</button>
...
<script>
    $(document).ready(function(e) {
        ...
        $("#selectImage").on('click', function(e) {
            if (editor_image == true) {
                var url = "{{url('fetch-editor-image')}}" + "/" + selected_image_id;
                $.ajax({
                    type: "get",
                    url: url,
                    success: function (response) {
                        var src = "{{static_asset('/')}}" + "/" + response;
                        console.log("res:" + src);
                        tinymce.activeEditor.execCommand('mceInsertContent', false, '<p><img src="' + src + '" alt=""/></p>');
                    }
                });
                editor_image = false;
                return;
