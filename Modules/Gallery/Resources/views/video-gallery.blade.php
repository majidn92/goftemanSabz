<div id="video-gallery" class="modal fade video-modal-lg" tabindex="-1" role="dialog" aria-labelledby="videoGallery" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ __('video_gallery') }}</h5>
                <button type="button" class="close mx-0" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true" class="float-left">&times;</span>
                </button>
            </div>
                ...
                <div class="row">
                    <div class="col-md-4 border-right">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                <label for="video" class="upload-file-btn btn btn-primary  btn-block"><i class="fa fa-folder input-file" aria-hidden="true"></i> {{ __('add_video') }}
                                </label>
                                <input id="video" name="video" type="file" class="form-control d-none" required onChange="videoUploadBtn()" data-index="0">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group" id="divVideoUploadBtn">
                                <div class="form-group text-center">
                                    <button type="submit" name="btn_video_upload" id="video-upload-btn" class="btn btn-primary btn-block"><i class="fas fa-cloud-upload-alt"></i> {{ __('upload') }}</button>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="load-more" id="load-more-video">
                            <a href="javascript:void(0)" class="">{{ __('load_more') }}</a>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" id="selectVideo" class="btn btn-primary ml-4" data-dismiss="modal">{{ __('select_video') }}</button>
...
<script>
    $(document).ready(function(e) {
                ...
                $('#videoUploadForm').on('submit', (function(e) {
                        ...
                        $("#video-upload-btn").html('<i class="fa fa-spinner fa-pulse fa-fw"></i><span class="sr-only"></span> در حال بارگذاری ...');
                        $.ajaxSetup({
                            headers: {
                        ...
                        $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    $("#video-library").prepend(
                                            ...
                                            $.notify('ویدئو با موفقیت بارگذاری شد', "success"); $("#video").val(''); $("#video-upload-btn").html('<i class="fas fa-cloud-upload-alt"></i> Upload'); $("#video-upload-btn").prop('disabled', false); $("#video_name").text(' '); $("#divVideoUploadBtn").hide();
                                        },
...
<script>
    ...
    $("#selectVideo").on('click', function() {
        ...
        var url = `{{url('fetch-video-selected/${selected_video_id}')}}`;
        $.ajax({
            type: "get",
            url: url,
