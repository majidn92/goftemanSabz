...
class RssController extends Controller
{
    ...
    public function importRss()
    {
        ...
        $activeLang     = Language::where('status', 'active')->orderBy('name', 'ASC')->get();
        $sections = Section::skip(3)->take(9999)->get();
        $states = State::all();
        return view('post::import_rss', compact('categories', 'subCategories', 'activeLang', 'sections', 'states'));
    }
    ...
    public function saveNewRss(Request $request)
    {
        ...
        try {
            ...
            $rssFeed->layout            = 'style_2';
            $rssFeed->save();

            if ($request->has('section_id')) {
                foreach ($request->section_id as $item) {
                    \DB::table('rss_feed_section')->insert([
            ...
            if ($request->has('state_id')) {
                foreach ($request->state_id as $item) {
                    \DB::table('rss_feed_state')->insert([
                        'rss_feed_id' => $rssFeed->id,
                        'state_id' => $item,
                    ]);
                }
            ...
            if ($request->has('category_ids')) {
                ...
                foreach ($category_ids as $item) {
                    \DB::table('category_rss_feed')->insert([
                        'category_id' => $item,
                        'rss_feed_id' => $rssFeed->id,
                    ]);
                }
            ...
            if ($request->has('sub_category_ids')) {
                ...
                foreach ($sub_category_ids as $item) {
                    \DB::table('rss_feed_sub_category')->insert([
    ...
    public function editRss($id)
    {
        ...
        if ($feed->category_id != "") {
            $subCategories  = SubCategory::where('category_id', $feed->category_id)->get();
        }
        $feed_sections_id = [];
        if ($feed->section->count()) {
            $feed_sections = $feed->section;
            foreach ($feed_sections as $feed_section) {
                array_push($feed_sections_id, $feed_section->id);
            }
        }
        return view('post::edit_rss', compact('feed', 'activeLang', 'categories', 'subCategories', 'sections', 'feed_sections_id', 'states'));
    }
    ...
    public function updateRss(Request $request, $id)
    {
        ...
        try {
            ...
            \DB::table('rss_feed_section')->where('rss_feed_id', $rssFeed->id)->delete();
...
// dd($item->description);
                    ...
                    $post->sub_title      = $item->description;
                    ...
                    if ($feed->category->count()) {
                        $sub_categories = $feed->subCategory;
