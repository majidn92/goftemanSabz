...
@section('content')
    ...
    <div class="dashboard-ecommerce">
        ...
        <form id="delete-post-from" action="{{ url('post/delete-post-from') }}" method="post" style="display: inline">
            ...
            <input type="hidden" name="column" value="breaking">
        </form>
                    ...
                    <div class="col-12">
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                            <tr id="row_{{ $post->id }}">
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['set_order_post']))
                                                <td width="10%">
                                                    <input type="number" class="form-control btn btn-light" name="order[]" value="{{ $post->breaking_order }}">
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['post_write']) && Sentinel::getUser()->hasAccess(['post_options']))
                                                    <td>
                                                        <a href="javascript:void(0)" onclick="remove_post_form('other','breaking','{{ $post->id }}')" name="option" class="btn btn-light active btn-xs">
                                                            <i class="fa fa-minus option-icon"></i>حذف از فوری
                                                        </a>
                                                    </td>
                                                @endif
                                            </tr>
                        ...
                        @if (Sentinel::getUser()->hasAccess(['post_write']) && Sentinel::getUser()->hasAccess(['post_options']) && $posts->count() > 0)
                            <button type="submit" name="btnSubmit" class="btn btn-primary pull-right">
                                <i class="m-l-10 mdi mdi-content-save-all"></i>{{ __('update_order') }}
                            </button>
                        @endif
                    </div>
                </div>
