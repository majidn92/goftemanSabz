...
@section('content')
    ...
    <div class="dashboard-ecommerce">
        <form id="delete-post-from" action="{{ url('post/delete-post-from') }}" method="post" style="display: inline">
            ...
            <input type="hidden" name="column" value="opinion">
        </form>

        <div class="container-fluid dashboard-content ">
            <!-- page info start-->
            {!! Form::open(['route' => 'update-opinion-order', 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                            ...
                                            @endif
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['set_order_post']))
                                                <td width="10%">
                                                    <input type="number" class="form-control btn btn-light" name="order[]" value="{{ $post->opinion_order }}">
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['post_write']) && Sentinel::getUser()->hasAccess(['post_options']))
                                                    <td>
                                                        <a href="javascript:void(0)" onclick="remove_post_form('other','opinion','{{ $post->id }}')" name="option" class="btn btn-light active btn-xs">
                                                            <i class="fa fa-minus option-icon"></i>حذف از دیدگاه
                                                        </a>
                                                    </td>
                                                @endif
                                            </tr>
