...
@section('content')
    ...
    <div class="dashboard-ecommerce">
        <form id="delete-post-from" action="{{ url('post/delete-post-from') }}" method="post" style="display: inline">
            ...
            <input type="hidden" name="column" value="chat_room">
        </form>

        <div class="container-fluid dashboard-content ">
            <!-- page info start-->
            {!! Form::open(['route' => 'update-chat-room-order', 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                            <tr id="row_{{ $post->id }}">
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['set_order_post']))
                                                <td width="10%">
                                                    <input type="number" class="form-control btn btn-light" name="order[]" value="{{ $post->chat_room_order }}">
                                                ...
                                                @if (Sentinel::getUser()->hasAccess(['post_write']) && Sentinel::getUser()->hasAccess(['post_options']))
                                                    <td>
                                                        <a href="javascript:void(0)" onclick="remove_post_form('other','chat_room','{{ $post->id }}')" name="option" class="btn btn-light active btn-xs">
                                                            <i class="fa fa-minus option-icon"></i>حذف از اتاق گفتگو
                                                        </a>
                                                    </td>
                                                @endif
                                            </tr>
