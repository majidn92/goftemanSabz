                                                ...
                                                <tr role="row" id="row_{{ $comment->id }}" class="odd">
                                                    ...
                                                    <td>
                                                        @if ($comment->created_at != null)
                                                            {{ miladi_to_jalali($comment->created_at,1) }}
                                                        @endif
                                                    </td>

                                                    @if (Sentinel::getUser()->hasAccess(['comments_delete']))
                                                        <td>
                                                            <a href="javascript:void(0)" class="btn btn-light active btn-xs" onclick="delete_item('comments','{{ $comment->id }}')"><i class="fa fa-trash"></i>
                                                                {{ __('delete') }}
                                                            </a>
                                                        </td>
                                                    @endif
