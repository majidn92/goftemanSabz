    ...
    <div class="entry-thumbnail" >
        <a href="{{ route('article.detail', ['id' => $post->slug]) }}">
            @if (isFileExist(@$post->image, $result = @$post->image->big_image))
                {{-- <img src="{{ safari_check() ? basePath(@$post->image) . '/' . $result : static_asset('default-image/default-1080x1000.png') }} " data-original=" {{ basePath($post->image) }}/{{ $result }} " class="img-fluid" alt="{!! $post->title !!}" height="300px"> --}}
                <img src="{{ static_asset($post->image->big_image_two) }} "  class="img-fluid" alt="{!! $post->title !!}" height="300px">
            @else
